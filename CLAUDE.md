...
## Offene Punkte (Stand: 2026-05-28)
...
| 2026-05-26 | Phase 13 Transport-Fix (3 Iterationen): (1) sonosRealAdapter.ts: `getPrimaryDevice()` statt `getPrimaryIP()`, zentrale `avTransport()`-Methode mit detailliertem Logging (action/room/ip), Fehler werden jetzt rethrown statt geschluckt. (2) playback.ts: `SONOS_REAL`-Konstante beim Modulload entfernt (war undefined weil dotenv NACH Imports läuft) → `isRealMode()` liest `getDiagnostics().mode` zur Laufzeit. Real mode: await Sonos + `{ ok:true, mode:"real", action }` Response. Mock mode: Mock-Playback-State wie bisher. (3) Frontend: `playbackApi.ts` — `transport()` helper erkennt Real-Mode-Response (`'ok' in r`) und gibt null zurück. `applyBackendState` akzeptiert nullable. `isRealTransportModeRef` in App.tsx — wird beim ersten null-Result gesetzt; verhindert danach, dass Mock-Timer-Poll `setIsPlaying` überschreibt. `handleTransportResult` ersetzt alle `.then(applyBackendState)` für Transport-Calls. |
| 2026-05-27 | Phase 14 Queue System Rework: Backend-eigene Queue-Architektur. QueueItem-Interface (id, albumId, trackId, trackIndex, title, artist, albumTitle, durationSeconds, durationFormatted, coverUrl, source). mockState.ts: queue: QueueItem[]. playbackService.ts: advanceToNext() (Queue-first, dann Album-Fortsetzung), buildQueue() (explicit + upcoming album tracks für Mock-Alben), NowPlayingResponse.queue[], addToQueue/removeFromQueue/reorderQueue, playAlbum() leert Queue. Neue Endpoints: GET /api/queue, POST /api/queue/add (mode: next|append), DELETE /api/queue/:id, POST /api/queue/reorder. Frontend: QueueItem-Typ in types/music.ts, del() in client.ts, addToQueue/removeQueueItem/reorderQueue in playbackApi.ts. NowPlaying.tsx: queue: QueueItem[] Prop, onQueueItemSelect statt onQueueTrackSelect — keine lokale Queue-Berechnung mehr. TrackMenu.tsx: onQueueNext (Als Naechstes) + onQueueAppend (In Warteschlange) getrennte Callbacks. AlbumBackside, AlbumCard, CoverFlow entsprechend aktualisiert. App.tsx: backendQueue-State aus 1s-Polling, handleQueueNext/handleQueueAppend → API, handleQueueItemSelect. Beide Builds fehlerfrei. |
| 2026-05-27 | Sonos NowPlaying Metadata Integration: In Real-Sonos-Mode rendert NowPlaying jetzt echte Metadaten vom Backend. Neues `CurrentTrack`-Interface in `playbackService.ts` (title, artist, album, durationSeconds, progressSeconds, coverUrl, uri, source). `getSonosAlignedState()` in `routes/playback.ts` gibt `current` aus `SonosPositionInfo` zurück — inkl. absoluter Cover-URL (http://<ip>:1400/...). Backend-Route sendet `current` im `/api/now-playing`-Response. Frontend: `CurrentTrack`-Typ in `playbackApi.ts`, `sonosCurrentTrack`-State in `App.tsx`, `applyBackendState` setzt `isRealTransportModeRef=true` und `sonosCurrentTrack` wenn `source==='sonos'`. NowPlaying.tsx: neuer `currentOverride`-Prop, `displayTitle/Album/CoverUrl` leiten in Real-Mode von `currentOverride` ab, Interpret bleibt `track.artist ?? album.artist`. Cover-Image zeigt echte Sonos-URL wenn vorhanden, sonst Premium-Gradient-Fallback (kein Text mehr in Cover-Fläche). Backend-Log: `[NowPlaying] source=sonos title="..." artist="..."` nur beim Track-Wechsel. Mock-Mode unverändert. |
| 2026-05-27 | Sonos Playlist-Kontext via AVTransport GetMediaInfo: Neues `SonosMediaContext`-Interface (contextType, contextId, contextUri). Adapter-Interface + Real-Adapter: `GetMediaInfo` → parst `CurrentURI` auf `spotify:playlist:ID` oder `spotify:album:ID`. Mock-Adapter: Stub returns unknown. sonosService.ts durchgereicht. `CurrentTrack` um `contextType`/`contextId` erweitert. `getSonosAlignedState()` ruft `getMediaInfo()` parallel zu Queue+Position. Frontend: `isNowPlayingPlaylist` reagiert auf `sonosCurrentTrack?.contextType === 'playlist'`. `useEffect` in App.tsx: wenn Playlist-Kontext erkannt → `setNowPlayingAlbum(contextId)` + `ensureAlbumDetails(contextId)` → Playlist-Name aus Spotify API geladen → NowPlaying Zone 1 zeigt korrekten Playlist-Namen. Progress-Tracking aus Sonos (GetPositionInfo) ist damit vollständig abgeschlossen — Phase-14-Punkt erledigt. |
| 2026-05-28 | NowPlaying Sonos Metadaten-Fixes (3 Iterationen): (1) Interpret-Fix: NowPlaying nutzt jetzt `currentOverride.artist` statt lokalem `track.artist`/`album.artist` in Sonos-Mode, mit Fallback falls Sonos-Metadata leer; (2) `extractXmlValue` robuster: matches jetzt auch Tags mit XML-Attributen (z.B. `<upnp:artist role="AlbumArtist">`), Artist-Fallback auf `r:albumArtist` + `dc:creator`; (3) HTML-Entity-Fix: `decodeXmlEntities()` in Backend — `&amp;` in Sonos-DIDL wird korrekt zu `&` dekodiert; (4) `contextTitle` aus Sonos `CurrentURIMetaData` via `indexOf`-Extraktion (robuster als Regex), nur gesetzt wenn `upnp:class` Playlist angibt; (5) Root Cause gefunden: Sonos meldet bei lokaler Queue `CurrentURI=x-rincon-queue:RINCON_...` statt Spotify-Playlist-URI — Playlist-Kontext damit aus Sonos allein nicht ermittelbar; (6) Lösung: Spotify `GET /me/player` als Fallback mit 15s Cache in `routes/playback.ts` — liefert `context.uri=spotify:playlist:ID` → `contextType=playlist`, `contextId` gesetzt → Frontend findet Playlist-Namen in `spotifyLibrary.playlists`; (7) `getDisplayedAlbumById` in App.tsx prüft jetzt auch `spotifyLibrary.playlists` (war bisher nur `.albums`). |
| 2026-05-28 | Sonos Play/Pause-Button-Bug gefixt: Pause-Button zeigte immer an obwohl Sonos nicht spielt. Root Cause: `isPlaying` startet mit `useState(true)` und wurde in Real-Mode nie aktualisiert (Guard `isRealTransportModeRef`). Fix: neues `getTransportInfo()` via UPnP AVTransport `GetTransportInfo` SOAP-Call (`CurrentTransportState === 'PLAYING'`). Methode in Adapter-Interface + SonosRealAdapter (+ Stub in Mock) + sonosService.ts. `getSonosAlignedState()` ruft es parallel; Backend überschreibt `isPlaying` im Response mit echtem Sonos-State. Frontend `applyBackendState` ruft `setIsPlaying` jetzt auch in Sonos-Mode. |
| 2026-05-28 | Playlist-Name in NowPlaying (3 Iterationen, unvollständig gelöst): Sonos liefert im `x-rincon-queue`-Modus keine Playlist-ID in `GetMediaInfo` — nur `object.container.playlistContainer` ohne dc:title oder Spotify-URI. (1) Spotify `/me/player`-Fallback: zuverlässig wenn Spotify Connect aktiv auf Sonos. Condition-Bug: Fallback wurde bei `contextType=playlist` übersprungen auch wenn `contextId` leer → gefixt auf `!resolvedContextId`. (2) Falscher Name: Spotify gab Kontext vom Handy ("Dark Jazz Playlist") — Spotify Connect nicht aktiv auf Sonos. Fix-Versuch: deviceType=Speaker-Check → zeigte nichts mehr. (3) Aktuell implementiert: Track-ID-Matching — extrahiert Spotify-Track-ID aus Sonos-URI (`x-sonos-spotify:spotify:track:TRACKID?...`), vergleicht mit Spotify `/me/player` item.id. Nur wenn Track-IDs matchen → Spotify Connect ist aktiv auf Sonos → Playlist-Kontext zuverlässig. `getCurrentPlaybackContext()` gibt jetzt `trackId` zurück. TTL 15s→5s für schnelleres Update. Status: noch nicht vom User bestätigt ob es funktioniert. |
